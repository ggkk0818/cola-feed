param(
  [string]$SourcePath = 'C:\Users\ggkk2\Downloads\FontCN24.c',
  [string]$HeaderPath = 'C:\Code\cola-feed\cola-epaper-i7\include\modules\display\FontCN24.h',
  [string]$CppPath = 'C:\Code\cola-feed\cola-epaper-i7\src\modules\display\FontCN24.cpp'
)
...
constexpr uint8_t kGlyphHeight = 46;
...
[void]$builder.AppendLine('#include "modules/display/FontCN24.h"')
[void]$builder.AppendLine('')
[void]$builder.AppendLine('namespace FontCN24 {')
[void]$builder.AppendLine('namespace {')

for ($index = 0; $index -lt $glyphs.Count; $index++) {
  $glyph = $glyphs[$index]
  if ($glyph.Bytes.Count -ne 184) {
    throw "Glyph $index ($($glyph.Char)) has $($glyph.Bytes.Count) bytes, expected 184."
  }
