param(
  [string]$SourcePath = 'C:\Users\ggkk2\Downloads\FontCN64.c',
  [string]$HeaderPath = 'C:\Code\cola-feed\cola-epaper-i7\include\modules\display\FontCN64.h',
  [string]$CppPath = 'C:\Code\cola-feed\cola-epaper-i7\src\modules\display\FontCN64.cpp'
)
...
[void]$builder.AppendLine('#include "modules/display/FontCN64.h"')
[void]$builder.AppendLine('')
[void]$builder.AppendLine('namespace FontCN64 {')
[void]$builder.AppendLine('namespace {')

for ($index = 0; $index -lt $glyphs.Count; $index++) {
  $glyph = $glyphs[$index]
  if ($glyph.Bytes.Count -ne 504) {
    throw "Glyph $index ($($glyph.Char)) has $($glyph.Bytes.Count) bytes, expected 504."
  }
